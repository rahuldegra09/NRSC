...
import { MapContainer, TileLayer, useMapEvents, GeoJSON, ImageOverlay, Popup } from 'react-leaflet';
...
function MapWithHoverAndClickHandler() {
    ...
    const svgUrl = process.env.PUBLIC_URL + '/indiapy.svg';
    const svg1Url = process.env.PUBLIC_URL + '/color.svg';
   
    const bounds = [
        [5.4, 39.7], // Southwest corner of India
        [41.6, 136.25] // Northeast corner of India
    ];
    const bounds1 = [
        [6.4, 38.7], // Southwest corner of India
        [38.6, 98.25] // Northeast corner of India
    ];
    ...
    return (
        <div className='p-3 gap-2 md:flex grid'>
            <div className='p-3 relative  items-center sm:w-full md:w-[60%] h-fit sm: rounded shadow-lg shadow-cyan-700 bg-gray-800 '>
                <MapContainer scrollWheelZoom={false} center={[20.5937, 78.9629]} zoom={4} style={{ height: '500px', width: '100%' }} >
                    <TileLayer
                    ...
                    {indiaGeoJSON && <GeoJSON data={indiaGeoJSON} onEachFeature={onEachFeature}  />}
            ...
            <div className='shadow-lg shadow-red-700 md:w-[40%] sm:w-full flex flex-col'>
              ...
              <div className=' p-1 flex flex-grow '>
                {!fetchedData && (
                        <div className="pt-1 text-gray-700 flex rounded shadow-orange-500 shadow-sm  w-full ">
